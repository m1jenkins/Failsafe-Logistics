...
import { 
  ...
  AlertCircle, ChevronDown, Check, Building, Phone, Plus, Minus,
  Clock, Truck, Globe, Cpu, Plane, Calendar, Activity, FileText, Briefcase
} from 'lucide-react';
...
export const ServiceLandingPage: React.FC<ServiceLandingPageProps> = ({ service, onNavigate }) => {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      const messageBody = `
        SERVICE PAGE REQUEST (${service.name})
        ------------------------------------
      ...
      const response = await fetch("https://api.web3forms.com/submit", {
        ...
        body: JSON.stringify({
          ...
          phone: formState.phone,
          message: messageBody,
          subject: `New Lead - ${service.name} Request`
        })
      });
  ...
  const inputClasses = (hasError: boolean) => `w-full glass-input text-slate-200 px-5 py-4 rounded-xl outline-none placeholder:text-slate-600 text-sm md:text-base border transition-all duration-300 font-sans ${
    hasError 
      ...
      : 'border-white/[0.04] focus:border-red-500/55'
  }`;
            ...
            <div className="glass-panel-elevated p-8 rounded-3xl shadow-2xl relative border border-white/[0.05]">
              ...
              <div className="mb-6">
                ...
                <p className="text-slate-400 text-xs mt-1 font-light leading-relaxed">
                  Enter pickup & delivery coordinates for an immediate response ticket.
                </p>
              </div>
              ...
              {isSubmitted ? (
                <div className="text-center py-10 space-y-4 font-sans" role="alert" aria-live="polite">
                  ...
                  <p className="text-slate-400 text-sm leading-relaxed max-w-xs mx-auto">
                    Your request has been dispatched to our central desk. A dispatcher will contact you by phone shortly to finalize details.
                  </p>
                  <Button 
                    variant="secondary" 
                ...
                <form onSubmit={handleSubmit} className="space-y-4" noValidate>
                  ...
                  <div className="pt-4">
                    ...
                    <p className="text-center text-slate-600 text-[10px] mt-3 leading-relaxed font-sans font-light">
                      Drivers route instantly upon dispatch telephone confirmation. Available 24 hours daily.
                    </p>
                  </div>
                </form>
              )}
